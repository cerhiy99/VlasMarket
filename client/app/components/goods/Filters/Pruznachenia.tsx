...
const Pruznachenia: React.FC<Props> = ({
  lang,
  realCountries,
  brand,
  currentPathname,
  ...
  useEffect(() => {
    const currentCountryIdsInUrl = searchParams.get('recognition');
  ...
  const handleCheckboxChange = (countryId: number) => {
    ...
    let currentCountryIds = searchParams.get('recognition')
      ? searchParams
          .get('recognition')!
          .split(',')
          .map((idStr) => parseInt(idStr.trim()))
    ...
    if (currentCountryIds.length > 0) {
      newSearchParams.set('recognition', currentCountryIds.join(','));
    } else {
      newSearchParams.delete('recognition'); // Видаляємо параметр, якщо немає вибраних країн
    }
  ...
  return (
    <div className="brands-container">
      ...
      <div className="brands-header" onClick={toggleDropdown}>
        ...
        <span>{lang == 'ru' ? 'НАЗНАЧЕНИЕ' : 'ПРИЗНАЧЕННЯ'}</span>
