...
const Linies: React.FC<Props> = ({
  ...
  useEffect(() => {
    const currentBrandIdsInUrl = searchParams.get('linia');
  ...
  const handleCheckboxChange = (brandId: number) => {
    ...
    let currentBrandIds = searchParams.get('linia')
      ? searchParams
          .get('linia')!
          .split(',')
          .map((idStr) => parseInt(idStr.trim()))
    ...
    if (currentBrandIds.length > 0) {
      newSearchParams.set('linia', currentBrandIds.join(','));
    } else {
      newSearchParams.delete('linia'); // Видаляємо параметр, якщо немає вибраних брендів
    }
