...
const OrdersPage = ({ params }: { params: Promise<{ lang: Locale }> }) => {
  ...
  const setStatus = async () => {
    try {
      console.log(4234234, selectedOrderIds, selectStatus);
  ...
  const setProcent = async (e: any, orderId: number) => {
    try {
      if (e.key === 'Enter' && !e.shiftKey) {
        e.preventDefault(); // блокує перенесення рядка
        const value = (e.target as HTMLTextAreaElement).value;
        const procent = value.toLowerCase().includes('полная') ? 3 : 2;
        const res = await $authHost.post('order/setProcent', {
          orderId,
          procent,
        });
        e.target.value = '';
          ...
          {filteredOrders.map((order: any, index) => (
            ...
            >
              ...
              <div className="coment-meneger">
                <textarea
                  onKeyDown={(e) => setProcent(e, order.id)}
